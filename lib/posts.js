...
export function getPostDataForRSS() {
  const fileNames = fs.readdirSync(postsDirectory);
  const allPostData = fileNames.map((singleFile) => {
    const fullPath = path.join(postsDirectory, singleFile);
    const fileContents = fs.readFileSync(fullPath, "utf8");
    const slug = singleFile.replace(/\.mdx$/, "");
    const matterStuff = matter(fileContents);
    return {
      ...matterStuff,
      slug,
    };
